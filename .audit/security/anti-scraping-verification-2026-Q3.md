...
**Canonical target (future):** <https://aisafeatwork.org/>
...
**Test 1 status:** **NOT YET RUN** — first execution scheduled after canonical domain (`aisafeatwork.org`) resolves + Google + Bing have had 4 weeks to index.
...
| 4 | `/robots.txt` | `Sitemap: https://aisafeatwork.org/sitemap.xml` line present | **PASS** (note: sitemap URL hard-codes canonical domain, not Netlify deploy URL — intentional for canonical move) | |
...
| Finding | Action |
|---|---|
| Canonical domain `aisafeatwork.org` not yet registered → sitemap + JSON-LD URLs point at a non-resolving domain | Register domain + DNS → Netlify. Until then, Netlify deploy serves; URLs in metadata are aspirational. |
...
- Canonical domain `aisafeatwork.org` live + Cloudflare-fronted
