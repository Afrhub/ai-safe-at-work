// A new manager's first day, end to end through the real UI against production:
//
...
export async function run() {
  ...
  if (!NEWMANAGER || !FREEAGENT) { await check("MGR", "fixtures configured", async () => skip("need E2E_NEWMANAGER_* and E2E_FREEAGENT_* in .env.e2e")); return report("e2e-manager-onboarding"); }
  ...
  await check("MGR-00", "the new manager starts with no authenticator", async () => {
    ...
    resetOk = true; ok(true, `removed ${removed} stale factor(s)`);
  });
