...
5. **Carbon: monochrome silver ink on dark paper — the Audit Dossier language.** Ruled hairlines, crop-mark corners, file tabs, clause numbering, classification chips, watermarked citations; slow layered parallax (blueprint grid + ledger rules + clause watermarks). **Colour appears only as signal**: green = practice/pass, red = caution/prohibited, blue = informational/sector overlays. No brand accent colour — distinctiveness comes from restraint. (Carbon palette 2026-06-11, replacing amber after founder flagged it as Anthropic-adjacent. Typography: Atkinson Hyperlegible Next / Mono site-wide for low-vision legibility — body ≥17px, no tight tracking.)
