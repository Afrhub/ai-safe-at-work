...
| 6 | **Role-based tracks** (board, manager, IC, DPO, dev) | Different roles → different obligations | 2–3 wk | **PARTIAL 2026-06-03** — Manager track + DPO track shipped v2026.05. **MSP / IT Admin track shipped today** as `/module-msp-admin.html` (10 sub-modules covering Art 4 own-staff + Art 26 customer side + baseline-at-scale + approved-tool inventory + customer onboarding + patch/change management + audit-pack handover + first-10-min incident response + white-label posture + 10-question assessment). Per outline at `.audit/course-quality/role-track-outlines/msp-it-admin.md`. Remaining: Microsoft Copilot, Shadow AI, Governance & Procurement, AI Governance Officer, Developer/Technical AI (outlines for all five exist; build cadence TBD). Current: 3 of 6 planned role tracks live. |
...
**Status as of 2026-06-03 (end of day)**: gates 1, 2, 3, 4, 5, 7, 9, 11, 13, 14 closed. Gate 6 advanced from 2/6 → 3/6 (MSP/IT Admin track shipped today). Gate 12 partial (recipe ready, purchase blocked on user financial action). Gate 8 (WCAG audit) partial. Week-one block fully closed; quarter-one block reduced to gate 6 remainder (3 role tracks: Microsoft Copilot, Shadow AI, Governance & Procurement / AI Governance Officer / Developer-Technical). Procurement-side: gate 15 (CPD accreditation — external multi-month) + gate 10 (sector overlays — multi-week per vertical) remain. Strong position for first MSP-partner activation; MSP-side track now provides RORtech-equivalents an out-of-the-box provider-side AI literacy baseline.
