...
| Date | Decision | Why |
|---|---|---|
| 2026-09-02 | **Runbook Phase 4 and the machine half of Phase 5 executed by API.** Form notifications routed to the verified account mailbox (James@attest-ai.com cannot receive: no MX); orphan forms and test rows deleted. `invite-seat` transcribed into the repo, reviewed twice, redeployed v3: invite link on attest-ai.com, CORS pinned, no orphaned invitee on seat failure, manager/reseller emails refused. Invite → email → enrol → module → roster proven end to end with test accounts. | Nothing JC meets is untested; his first sign-in is the only step a machine must not do for him |
| 2026-09-02 | **Launch runbook Phases 1–2 executed.** `attest-ai.com` live on Netlify (cert issued 1 Sep), DNS at 123-Reg (registrar confirmed via RDAP; nameservers are GoDaddy-group). Resend domain verified (DKIM/SPF/MX). Supabase Auth on Resend SMTP as `no-reply@attest-ai.com`, 30 emails/h, `smtp_max_frequency` 10 s, redirects for attest-ai.com/www/netlify.app — applied by `scripts/phase2-supabase-auth-config.mjs`. Proof: real reset email delivered to a human inbox. Test board now targets attest-ai.com. | First time the product could email anyone; unblocks self-serve resets, invites, JC's staff onboarding, and Stripe fulfilment mail |
...
| 2026-07-19 | **Pricing council (two sessions, Karpathy LLM-council method: 5 advisors + 5 anonymous peer reviews + chairman) — pricing structure locked for launch.** Session 1 (monetisation): platform priced per ORGANISATION in headcount bands, never per user (org-level value; 2-seat gaming; MSP margin arithmetic); the £1,000 one-off-with-everything cannibalises any subscription — restructure to annual "Foundation"; publish numbers (a blank price loses every visitor — transparency's real payoff is self-qualification of founder time); RORtech approached as design partner, founding promise NOT honoured until 70/30 maths reconciled against their client book; consultancy = first-revenue + price-discovery engine. Session 2 (bands/transparency): fine bands (1-20/21-50/51-100/101-150) rejected unanimously — invented precision with zero customers, manual-invoicing boundary events, repricing liability. **Locked grid: one table, three rows, two priced — Foundation £990/yr up to 25 staff / £1,750 up to 100; Platform £249/mo up to 25 / £499/mo up to 100 (billed annually £2,490/£4,990); 100+ = Contact us for both tiers (a public top-band number caps what large orgs would pay).** Both tiers shown at every row (a missing cell reads "call us" and kills the tab). Footnotes required: who counts (employees incl. part-time + contractors with access; measured at purchase; honour-system annual true-up), prices ex-VAT, band price grandfathered 12 months. MSP pricing stays partner-only (like rebates) — one "Partner pricing available" line; contract rule: partner resale price never exceeds public list. Blind spots logged: solo delivery capacity is the pricing ceiling; PI insurance + liability terms before first invoice; VAT £90k threshold; treat the page as an experiment — log the headcount behind every Contact-us enquiry to draw future bands. Full verdicts in `.audit/council/` (untracked — public repo). | Two independent councils converged on the same structure from different angles; every number now has an argued rationale and a named revisit trigger instead of a guess. |
| 2026-07-21 | **Competitive pricing validation (web-sourced) — Tier 2 confirmed, held conservative deliberately.** Comparators: DataGuard (closest analogue, GDPR platform + experts) from ~€175/mo, typical deals €2k-20k/yr; Vanta ~$10k/yr entry; Drata ~$7.5-15k/yr; KnowBe4 training ~$20-30/user/yr list; CyberSmart listed £49-99+VAT/user/mo (treated cautiously); Cyber Essentials cert from ~£320. Verdict: Platform at £4,990/yr for 100 staff = **£50/head/yr including all-staff training**, vs £6-12k for compliance platforms with NO training — 40-60% under the category while carrying the differentiator (content updated at least quarterly as the EU AI Act/standards move). Decisions: keep £249/£499 at launch (the discount IS the zero-customer wedge); **£599 for the 26-100 band banked as the post-validation move** (after first 5-10 customers); quarterly-update line goes ON the pricing grid under Platform; optional "Founding customer" banner (first 10: 20% off, price locked 2 yrs, named design partners); a "1-10 staff £590/yr" micro-row only if real too-expensive objections demand it. PDF of the full assessment: `.audit/council/attest-ai-pricing-assessment-2026-07-19.pdf` (untracked). | Prices moved from council assumption to market evidence; the only deliberate deviation from market rate (staying cheap) is a named wedge with a named exit. |
| 2026-07-21 | **Certification-claims analysis + landing repositioning ("journey" framing, not certification).** Portal content audit against certifications: ISO/IEC 42001 — strongest coverage (AIMS manual to clauses 4-10 + charter/ToR/RACI/registers/internal-audit/monitoring, roughly 60-70% of the documentation burden) but certification requires an OPERATING management system audited by a UKAS-accredited body — documents are the scaffold, not the certificate; ISO 27001 — citations only, no ISMS/SoA, no claim made; EU AI Act — best honest story: no deployer certificate exists, compliance is evidenced, and Art 4 literacy records + registers/DPIA/FRIA are exactly that evidence; Cyber Essentials — N/A. Standing copy rule: **"audit-ready" may only ever mean "records organised the way an auditor asks for them", never "you will pass an audit"; Attest AI never presents as a certification/accreditation body.** Landing page updated (commit `160c9ad`): hero lede reworded to "take AI governance seriously... at every step of your AI compliance and AI governance journey"; preface strip added under the hero ("A note on what we are. Attest AI is not a certification or accreditation body..." and prepares you to work with an accredited body); one overclaim softened. Terms.html already aligned (completion certificate is not ISO certification). Open: carry the same one-liner to course/pricing/portal pages. | Overclaiming certification is the one liability that ends a governance company (pairs with the council's PI-insurance warning); the journey framing is both safer and truer to what the product does. |
| 2026-07-21 | **Partner commercials moved behind the portal login.** `msp.html` was fully public (noindex only, no auth) and carried the entire partner economics: rebate tier table with wholesale-per-seat and margin columns, Governance Pack margin table, annual-margin curve, plus "flat 15% reseller discount / up to 50% effective" and the 15-20% referral fee in the body copy, meta description, OG/Twitter cards and JSON-LD. The gated `portal/reseller.html` linked *back out* to the public page for the numbers — that was the leak path. All three tables moved into `portal/reseller.html` (behind portal login); public page keeps the qualitative programme pitch (deal registration, NFR licences, co-branding, referrals) and points approved partners at the portal. NFR table retained publicly: free-licence counts only, no pricing. Doctrine rule confirmed: **partner pricing, rebates, wholesale and margin figures never appear on a public page — one "Partner pricing available" line is the public limit.** Commit `ef25deb`. | Public wholesale pricing lets any prospect compute the floor and negotiate to it, and hands competitors the channel model; the gated page linking out to it made the login theatre. |
| 2026-07-21 | **Frameworks + Resources parked out of the top nav.** Both removed from `.topbar-nav` across 76 pages (root, `portal/`, `v2/` incl. de/fr) pending a decision on placement; `standards-map.html` and `resources.html` remain live and reachable (verified 200). Exact markup, nav position and relative-path caveat recorded in `docs/nav-parked-links.md` — restore is copy-paste or a revert. Open: whether they land in a footer group, a Products dropdown, or back in the nav. | Nav was carrying six items plus three pills; parking beats deleting when the placement decision is still open. |
| 2026-07-21 | **Pricing page rebuilt on the locked grid + buyer journey aligned (the doctrine-to-page gap closed).** Audit found `/pricing` predated all three July council rows: still three numbered tiers, still the £1,000 one-off-with-everything, and **Platform showing no price at all** — the exact failure both councils named. Rebuilt to one table, three rows, both tiers priced at every row (Foundation £990/£1,750 per year; Platform £249/£499 per month, £2,490/£4,990 billed annually; over 100 staff Contact us for both), with all three mandated footnotes now on the page (who counts as staff incl. part-time + contractors with access, measured at purchase, honour-system annual true-up; prices ex-VAT; band price held 12 months) and the quarterly-content-update line on the grid. Tier numbers replaced with product names. **Certification rule carried onto the page**: "A note on what we are" preface under the hero, worded as *we get you AI governance and compliance ready*; "audit-ready reporting and evidence" softened to "reporting that organises your records the way an auditor asks for them"; bare "ISO 42001 Readiness Assessment" reframed as readiness support *preparing you to approach an accredited certification body*. MSP/partner-ladder copy removed from the page body and meta. Then swept the whole buyer journey, which still sold the old price: `checkout.html` rebuilt as the Foundation order (free-text team size replaced with a **required headcount band selector** so every order carries its band), Netlify form renamed `tier1-order` → **`foundation-order`**, "Plus Pack" dropped from the Foundation offer in favour of starter templates, CTAs updated on course/about/solutions, and `checkout-thanks.html`, the FAQ cost/plans answers (+ their JSON-LD) and `llms.txt` rewritten — `llms.txt` now also states the not-a-certification-body line for crawlers. Commits `8ad8cd0`, `521ded4`. **User action: set Netlify email notifications for `foundation-order`** (the old `tier1-order` form is now orphaned; nothing on the site emails anyone yet). Open: carry the certification preface to `course.html` and the portal; "Become a Partner" nav pill + footer "Partner Programme" still site-wide, pulling them is a commercial call. | A locked price that never reaches the page is a decision that didn't happen; the CTA and checkout contradicting the grid is worse than the old price, because it breaks trust at the moment of purchase. |
| 2026-07-25 | **Headcount bands changed: 1-25 / 26-50 / more than 50.** Supersedes the band structure locked 2026-07-19 (up to 25 / 26-100 / over 100). **Prices are unchanged** &mdash; Foundation &pound;990/&pound;1,750, Platform &pound;249/&pound;499 &mdash; so the middle band now covers **half the range it did**, and the Contact-us threshold **halves from 100 to 50**. Founder decision, taken directly, not council-tested. Consequences worth watching: the &pound;1,750 / &pound;499 band is now much better value per head at 50 staff than it was at 100, and everything above 50 becomes a conversation rather than a self-serve purchase &mdash; which raises the volume of quote requests against a solo delivery capacity that DOCTRINE 2026-07-19 already named as the real pricing ceiling. Applied across pricing.html, checkout.html (including the order form's band selector), plus-pack.html, faq.html and llms.txt. The banked &pound;599 post-validation move (2026-07-21) was priced against a 26-100 band and needs restating before use. | The published grid and the doctrine must never disagree; a price a customer can read is the price, so the decision log follows the page rather than the other way round. |
| 2026-07-25 | **Content posture inverted: gate the doing, not the knowing.** Audit found the site was not short of content but short of *reachable* content &mdash; 76 pages existed, a stranger could reach about ten. Four compounding faults: (1) `pricing.html` carried `noindex`, so the grid both councils argued for was invisible to search, as were faq, solutions, glossary and resources; (2) **20 gated pages invited indexing**, sending crawlers to a checkout redirect, which produces soft-404s and reads as cloaking &mdash; 48 gated pages now declare `noindex`, which is simply true; (3) **29 of the sitemap's 41 entries were gated pages** while every public marketing page but the homepage was missing, so Google was told to crawl the locked content and ignore the sellable content &mdash; rebuilt to 21 real URLs, priority by commercial intent; (4) `course.html` was public but linked from nowhere, so the flagship product had no route to it. Un-gated per the pricing council's trust recommendation: glossary, standards map, **module 1 as a free sample** (framed as such, with its end-of-module hand-off turned into an honest Foundation upsell) and the resources hub. `plus-pack.html` rebuilt as the **Attest AI Platform page** &mdash; it was selling a retired product name and nothing anywhere explained what the Platform does. **Standing rule: reading a glossary, browsing a blank template or seeing which ISO clause we map to is not the product; filling registers in, tracking them over time, proving staff completed it and generating the certificate is.** | Gating everything protected revenue that does not exist yet from an audience that could not reach us; the content *is* the marketing until there are customers. |
| 2026-07-25 | **Product videos: record the real thing, never recreate it.** Two narrated walkthroughs shipped &mdash; the platform (60s) and course module 1 (43s) &mdash; both **captured from the live product with Playwright**, then framed in Remotion (title, captions, end card) and narrated with ElevenLabs using the **win2linux house voice `lUTamkMw7gOzZbFIwmq4`, model `eleven_multilingual_v2`** and its voice settings, so both products sound like one company. About 27p total. **Rule: never rebuild the UI inside the video.** A recreation is a drawing of the product that goes stale the moment a button moves; a capture cannot drift from the build. Caption and narration timings come from the recorder logging its own cue points, never from eyeballing frames, and every narration line is measured against its slot before rendering. Source lives in `video-project/` (gitignored, ~700MB); only the rendered mp4 and poster ship. Regenerate narration with `ELEVENLABS_API_KEY=... node generate-voiceover.mjs` &mdash; it skips lines already made, so an edit re-bills only that line. Videos sit **behind the demo and course buttons**, play on every click, and a SKIP appears once the video has actually been watched through. **Two product bugs were found only by watching the footage**: the dashboard rendered ", AI Governance Dashboard" whenever the company name was blank (every customer on day one), and two calls to action fired simultaneously when a preview ended. | Watching your own product being used finds things no test asserts; recording rather than recreating means the marketing cannot quietly diverge from the software. |
| 2026-07-25 | **Certification overclaim found inside the paid course.** The MSP/IT Admin track promised learners they would "hand over an audit binder a customer's regulator would accept" &mdash; an outcome guarantee, stated twice, once as a course learning objective, i.e. taught to partners as a selling point. Reworded to "organised the way a regulator asks for it", with the same treatment for "audit-grade evidence" (module 9) and "audit-grade records" (course.html). Also fixed: `course.html`'s JSON-LD still advertised **&pound;1,000 as a one-off purchase** &mdash; invisible on the page but machine-readable and served to search engines and AI crawlers, contradicting the button beside it; and sub-module 9 of the MSP track ran entirely on a **retired partner tier structure** (Growth / Strategic / Schedule D) that does not exist in the live programme, quiz included. **The standing copy rule (2026-07-21) must be enforced inside course content, not just marketing pages** &mdash; the overclaim survived the landing-page repositioning because nobody swept the modules. | The riskiest certification claim was not on a sales page but taught as a course outcome, which is worse: it is the claim a partner repeats to their own client. |
| 2026-07-26 | **&ldquo;Who We Help&rdquo; page built; the audience nav finally leads somewhere.** The homepage `#who-for` section was six ticks and a heading, and the nav item **Business Types** pointed at it, so the one link a buyer clicks to check &ldquo;is this for an organisation like mine?&rdquo; led to almost nothing. New `/who-we-help.html`: seven audience types (small business, mid-market, growing, regulated, public sector suppliers, professional services, technology &amp; IT), each as the challenges they arrive with beside what we do about them, plus a **typical starting point** table mapping organisation to Foundation / Platform / Consultancy. Prices, plan names and the not-a-certification wording are pulled from `pricing.html` rather than restated, so they cannot drift. Nav renamed **Business Types &rarr; Who We Help** and repointed across 76 existing pages; homepage section kept but now links onward and gained the two audience types it was missing. Sitemap entry at 0.85. **Finding worth acting on separately:** the three sector overlays (`sector-financial-services.html` &mdash; 51k of DORA / SR 11-7 / Consumer Duty detail &mdash; plus healthcare and public sector) are all `noindex, nofollow` and reachable only from `resources.html`; `solutions.html` is indexed but linked only from `checkout.html`. The per-sector SEO surface is already built and switched off. Left as-is deliberately: flipping it is a positioning call, not a bug fix. Commit `cdd00f1`. | People buy when they recognise themselves. A nav item that promises audience fit and delivers a tick-list spends the click and returns nothing. |
| 2026-07-26 | **Top bar overflowed every page on mobile; fixed at the cause, not the symptom.** Reported as the theme toggle mispositioned. It is not: `.theme-toggle` is `position: relative`. The bar needed **457px of content at 390px wide and 823px at 768px**, so it never fitted &mdash; flex crushed the pills into discs (&ldquo;Sign / in&rdquo; over three lines) and pushed the toggle off the right edge, giving every page **67px of horizontal scroll**. Fixed by letting the bar and nav wrap (no breakpoint has to guess where that starts), `white-space: nowrap` + `flex-shrink: 0` on pills so they keep their shape, the brand logo down from **101px to 2.6em below 900px** (it is set inline as `height:6em` on every page, so this needs `!important`), and a tighter nav gap below 1100px to stop non-shrinking pills wrapping the bar at 1024 where it used to fit. The toggle now appends inside `.topbar-nav` rather than beside it, so it travels with the pills when the bar wraps. Also `.module-grid`, whose `minmax(300px, 1fr)` floor pushed 8px of scroll at 320px on `course.html`. Verified with Playwright: `scrollWidth - clientWidth` **0** at 320 / 390 / 768 across six pages, toggle in-viewport, tappable and still persisting the theme; desktop unchanged at one row, 124px. Cache-busters bumped **together** &mdash; `style.css` v25&rarr;v26 and `cinema.js` v21&rarr;v22, 84 references each, none mismatched. Commit `2ae5f94`. | A brief that names the symptom will get the symptom fixed. The bar had never fitted on a phone; the toggle was just the last thing pushed out, and moving it would have hidden that. |
| 2026-07-26 | **Portal review acted on; three of its items were description problems, not gaps.** External review of the platform. The **Use Case Register already collected** the tool, the business owner, the data categories and the purpose &mdash; only its guide text called it &ldquo;every approved AI tool in use&rdquo;, which is the one thing a use case is not (ChatGPT is the tool, writing proposals is the use case, and one tool carries several). **Lessons Learned already existed** on incidents. **Objectives already existed** as a template, just unsurfaced. Shipped: Vendor Due Diligence leads with structured diligence based on recognised frameworks rather than a question count (&ldquo;nobody buys 19 questions&rdquo;); **Dashboard &rarr; Governance Centre** in nav, heading and every guide pointing at it; **residual risk** on the Risk Register &mdash; likelihood and impact once mitigated, scored on the same matrix as the inherent risk, **blank until planned** and rendered &ldquo;Not scored&rdquo; because an unscored residual is a question outstanding, not a low risk; **business objectives as a Governance Centre widget** rather than another module, with overdue computed from the target date and never stored; the governance **chain drawn** on `plus-pack.html` as eleven steps from supplier to Governance Centre, kept as an `&lt;ol&gt;` so it degrades to a correctly ordered list and a screen reader gets the sequence; **&ldquo;eleven working modules&rdquo;, not sections**. Verified by driving the running portal, not by reading the diff: residual scores Low &times; Medium &rarr; Low and survives a reload; objectives add, badge overdue off a past target, and persist. `aimp.js` v20&rarr;v21; `style.css` / `cinema.js` untouched this round so no site-wide sweep. Commit `24d3b25`. | Review feedback is a hypothesis about the product, not a description of it. Half of these were the platform describing itself badly, which is cheaper to fix and would have been buried under building the features it already had. |
| 2026-07-26 | **Tagline deliberately not changed.** The review proposed replacing &ldquo;The AI Governance Platform, adopt AI safely, securely and responsibly&rdquo; with either &ldquo;Making AI governance practical&rdquo; or &ldquo;AI Governance. Connected. Continuous. Practical.&rdquo; Founder decision: **keep the existing line.** Recorded so it is not re-litigated. | &ldquo;Safely, securely and responsibly&rdquo; runs through the hero, the meta descriptions on 47 pages and the new Who We Help hero; changing the footer alone would have split the thread, and the review offered two candidates without picking one. A positioning line is not a copy edit. |
| 2026-07-26 | **Internal docs were being served publicly; blocked by shape, not by name.** Found while checking whether pushing a doctrine edit would publish it. `DOCTRINE.md` and `README.md` were blocked by name on 2026-05-19, but the rule was never generalised &mdash; so **17 internal files returned 200** to anyone who guessed the path, including **`/SCOPE.md`** (the full consultancy ladder &pound;4,995 &rarr; &pound;30,000+, the &pound;1,500&ndash;&pound;5,000/mo fractional retainer, **and** the MSP tiers &pound;499 / &pound;1,499 / &pound;3k&ndash;&pound;5k), `HANDOFF.md`, all of `docs/` (including **`ownership-and-exit-plan.md`** and the security architecture one-pager) and all of `specs/`. `.audit/` was correctly blocked throughout. Fixed in `netlify.toml` with `/*.md`, `/docs/*` and `/specs/*` &rarr; 404 force, deliberately matching on **shape rather than filename** so the next internal doc is covered without anyone remembering this file exists. Exposure window not determined: these paths have existed since the repo went live on Netlify, and Netlify does not retain per-path logs at this plan level. **Directly contradicts the 2026-07-21 decision** to move partner commercials behind the portal &mdash; the same numbers were readable at a public URL the whole time. | Blocking two files by name is not a policy, it is two exceptions. The exit plan and the partner ladder being one guessed URL away is the kind of leak that is discovered by the wrong person, not by an audit. |
| 2026-07-26 | **Correction to the row above: the exposure was the whole repository, and the first fix did not work.** The initial pass blocked `/*.md`, `/docs/*` and `/specs/*`. Deploying it and **checking the live URLs rather than assuming** showed `/docs/*` and `/specs/*` 404ing correctly while `SCOPE.md` and `HANDOFF.md` stayed 200: a Netlify splat only matches a **trailing path and cannot carry an extension suffix**, so `/*.md` matches nothing. That is almost certainly why DOCTRINE and README were blocked by name in the first place. A full sweep of every tracked file then showed the problem was never about markdown &mdash; Netlify publishes the repo, so also live were **`supabase/migrations/*.sql`** (the schema including the RLS-hardening and definer-lock migrations), the entire **`tests/`** suite, **`scripts/`**, both Remotion source projects **including the narration MP3s for the paid modules m2&ndash;m12**, **`AI-SafeWork-MSP-Partner-Deck.pptx`** and **`AI_SafeAtWork_Course_Content_v1_Word.docx`**. Now blocked: `/.audit/*`, `/docs/*`, `/specs/*`, `/supabase/*`, `/tests/*`, `/scripts/*`, `/video-m1/*`, `/video-m2/*` plus `SCOPE.md`, `HANDOFF.md`, `v2/scorm/README.md`, `skills-lock.json`, the deck and the docx. Verified first that the course plays its videos from `assets/video/`, not from the Remotion projects, so blocking those breaks nothing. `LICENSE*`, `changelog.json` and `assets/video/*` deliberately stay public. | Two lessons, both cheap to write down and expensive to relearn. A redirect that deploys successfully is not a redirect that works &mdash; only the live URL proves it. And &ldquo;block the internal docs&rdquo; framed the problem as documents when the real shape was *the build publishes everything I have not excluded*, which is the question worth asking of any static host. |
| 2026-07-26 | **AI Tool Register shipped: govern the accounts nobody bought.** External review asked how the platform handles staff using personal ChatGPT / Claude / Gemini accounts. Checking the platform rather than the description found a real hole, slightly differently shaped than described: **Vendor Due Diligence was the only front door, and a personal account has no supplier to send a questionnaire to**, so it could never enter the chain. `tool` on a use case was free text and `status` meant Active / Paused / Discontinued &mdash; an operational lifecycle, not an approval &mdash; so there was nowhere to say &ldquo;ChatGPT Personal, restricted, no company data&rdquo;, the commonest real-world SME case. Shipped a second front door: an **AI Tool Register** whose point is the account-type field, so the same product on an enterprise account and on a personal one are two rows with two different answers. A use case's tool became a reference to it, with anything typed beforehand kept and labelled &ldquo;not on the register&rdquo; rather than dropped. **Policy Section 3 now generates from the register**, so the rule staff read and the decision the business recorded cannot drift, falling back to the typed list when the register is empty. **Refused one item from the review**: a &ldquo;7 users still using unapproved AI&rdquo; dashboard widget. The platform cannot know who signed into ChatGPT with a personal address &mdash; that needs an endpoint agent and is a different product category &mdash; and inventing a compliance figure is the same overclaim caught inside the course on 25 Jul. The Centre instead reports what is sourced from rows someone wrote: a use case pointing at an uncleared tool, or at one already ruled out. Course side was already covered by `module-shadow-ai.html`; it was only the registers that could not hold the answer. Declaration workflow scoped, not built: `specs/ai-tool-declaration.md`. Commit `88be557`. | The distinction the review drew is the right one and worth stating in doctrine: security software asks whether it can stop someone; governance software asks whether the rules were defined, the risk assessed, the people trained, the decision recorded and the evidence kept. Every feature request should be tested against which of those it is, and technical enforcement stays out of the platform and inside consultancy. |
