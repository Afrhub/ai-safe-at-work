...
5. **Periwinkle Blueprint on carbon — the Audit Dossier language.** Ruled hairlines, crop-mark corners, file tabs, clause numbering, classification chips, watermarked citations; slow layered parallax (blueprint grid + ledger rules + clause watermarks). **Structural accent: periwinkle #91a2ff** on chips, clause numbers, file tabs, rules, labels and interactive states — never on body text, never on heading emphasis (headings are a single colour, always). Body ink stays silver. **Signals keep meaning**: green = practice/pass, red = caution/prohibited, blue = sector overlays. Banner line-art and background drawings stay silver ink; background light is Spectra (chromatic, scroll-shifting). (Carbon palette 2026-06-11, replacing amber after founder flagged it as Anthropic-adjacent. Typography: Atkinson Hyperlegible Next / Mono site-wide for low-vision legibility — body ≥17px, no tight tracking.)
